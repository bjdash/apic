!function($){
	...
	function createElement(key, value, type, expanderClasses) {
		...
		if(type == 'object') {
			...
			return object + '</ul><span class="js-close">' + close + '</span></li>';
		}
	...
	$(document).on('click', '.jjson-container .expanded', function(event) {
            ...
            var children = $(this).siblings('ul').children('li').length;
            var plural = children === 1? '' : 's';
            $(this).siblings('.js-close').prepend('<span class="js-items">'+children+' item' + plural +'...</span>')
	...
	$(document).on('click', '.jjson-container .expanded.collapsed', function(event) {
		...
		event.stopPropagation();
		var $self = $(this);
                $(this).siblings('.js-close').children('.js-items').remove();
                console.log($(this).siblings('.js-close'));
