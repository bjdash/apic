...
(function () {
    ...
    <div class="js-row" ng-class="showDetailsPan?\'act-row\':\'\'" ng-init="showDetailsPan=false;ctrl ={expanded:true}">\
        ...
        <span ng-hide="($mode == \'list\' || $mode == \'static\')&& entity.root$$">\
            ...
            <span ng-show="entity._readOnlyKey">{{entity._key}} : </span>\
            <span ng-hide="entity.root$$ || entity._hideKey">\
                <input ng-readonly="$mode == \'static\'" class="model-key" type="text" ng-model="entity._key" ng-change="setDirty()" />:</span> <span uib-dropdown class="dropdown">\
                <button type="button" uib-dropdown-toggle class="btn btn-link btn-href model-selector dropdown-toggle"  ng-click="openMenu(entity,$event)">\
                    <span ng-repeat="t in entity._type">\
                        ...
                        <span ng-if="t===\'Array\'" ng-class="entity._items[0]._type[0]" class="unspecified">[ {{entity._items[0]._type[0]?entity._items[0]._type[0]:\'Unspecified\'}}<span ng-if="entity._items[0]._type[0]==\'$ref\'">  ({{entity._items[0]._path+($models[entity._items[0]._value].nameSpace|| entity._items[0]._value)}})</span> ]</span>\
                        <span ng-if="t===\'$ref\'" class="faint">({{entity._path + ($models[entity._value].nameSpace||entity._value)}})</span>\
            ...
            <div class="model-summary">\
                ...
                <span ng-show="entity._description" class="icon bj-playlist_add_check ico" uib-tooltip="{{entity._description}}"></span>\
        ...
        <div class="margL10" ng-if="entity._refExp && entity.root$$">\
            <div ng-init="entity=entity._refExp" ng-include="\'modules/JsonSchemeEditor/main-schema.html\'"></div>\
        </div>\
        <div class="margL10" ng-if="entity._items[0]._refExp && entity.root$$">\
    ...
    <div ng-show="ctrl.expanded" class="objProps">\
        <div ng-repeat="entity in entity._properties track by $index" class="propCont">\
                ...
                <div class="margL10" ng-if="entity._refExp">\
                    <div ng-init="entity=entity._refExp" ng-include="\'modules/JsonSchemeEditor/main-schema.html\'"></div>\
                </div>\
                <div class="margL10" ng-if="entity._items[0]._refExp">\
                    <div ng-init="entity=entity._items[0]._refExp" ng-include="\'modules/JsonSchemeEditor/main-schema.html\'"></div>\
                </div>\
        </div>\
        <div ng-repeat="entity in entity._additionalProperties track by $index" class="propCont">\
                <div ng-include="\'modules/JsonSchemeEditor/main-schema.html\'"></div>\
                <div class="margL10" ng-if="entity._refExp">\
                    <div ng-init="entity=entity._refExp" ng-include="\'modules/JsonSchemeEditor/main-schema.html\'"></div>\
                </div>\
    ...
    <div ng-show="ctrl.expanded" ng-if="entity._type.indexOf(\'Object\')<0 && hasChild.indexOf(entity._items[0]._type[0])>=0 " ng-repeat="prop in entity._items[0]._properties track by $index" class="propCont">\
        ...
        var ObjectSchema = '<div class="form-group col-xs-12">\
        ...
        <div>\
            ...
            <div ng-show="entity._additionalProperties.length==0">\
                <button class="btn btn-link c btn-sm block" ng-click="addAdditionalProp(entity)">Define Additional properties schema</button>\
                <div class="checkbox" style="padding: 2px 0;">\
            ...
            <div ng-show="entity._additionalProperties.length!=0">\
                <button class="btn btn-link c btn-sm red" ng-click="removeAdditionalProp(entity)">Remove Additional properties schema</button>\
            </div>\
        </div>\
        ';
        ...
        var modelSelectorSchema = '<div class="model-type-selector-cont">\
            ...
            <div ng-if="configs.showMoreOptn == \'$ref\' || configs.extraArrayOptn">\
                ...
                <div class="array-items">\
                    <select ng-click="$event.stopPropagation()" ng-change="setModelFor$Ref()" style="height:24px" ng-model="$parent.$parent.$parent.$parent.modelRef" ng-change="setDirty()" >\
                        <option ng-repeat="(id, m) in $models track by $index" value="{{m._id}}">#/definitions/{{m.nameSpace}} ({{m.name}})</option>\
                        <option ng-repeat="m in $responses track by $index" value="{{\'#/responses/\'+m.name}}">#/responses/{{m.name}} ({{m.name}})</option>\
                    </select>\
                </div>\
            </div>\
        </div>';
        ...
        return {
            ...
            link: function ($scope, ele, attr, model) {
                ...
                $scope.setModelFor$Ref = function () {
                    ...
                    if ($scope.modelRef.indexOf(resPath) === 0) {
                        selectedEntity._path = resPath;
                        selectedEntity._value = $scope.modelRef.substring(resPath.length);
                    } else {
                        selectedEntity._path = '#/definitions/';
                        selectedEntity._value = $scope.modelRef;
                    }

                    // if($scope.configs.extraArrayOptn){
                    //     $scope.selectedEntity._items[0]._value = $scope.modelRef;
                    // }else{
                    //     $scope.selectedEntity._value = $scope.modelRef;
                    // }
                };
                ...
                $scope.addAdditionalProp = function (entity) {
                    var additionalProperties = JsonSchema.newString('additionalProperties', false, entity._parent + '.' + entity._key.replace('##ROOT##', 'data'));
                ...
                $scope.removeAdditionalProp = function (entity) {
                    entity._additionalProperties = [];
                }

                $scope.buildTest = function (entity, event) {
                    $scope.configs.tbHelper.show = true;
                    $scope.configs.tbHelper.data.parent = entity._parent.replace('##ROOT##', 'data');
                    $scope.configs.tbHelper.data.key = entity._key;
                    var e = $(event.target);
                ...
                $scope.expandRef = function (entity) {
                    ...
                    if (!obj) return;
                    if (!obj._refExp) {
                        var model = $scope.$models[obj._value] || ($scope.$responses && $scope.$responses.find(function (a) {
                            return a.name === obj._value
                        }));
                        var schema = model.data;
                        var refData = JsonSchema.schema2obj(schema, '', undefined, true, $scope.$models, obj._parent + (obj._key ? ('.' + obj._key) : '')); //obj._key is blank fro Array[$ref]
                ...
                switch (type) {
                    case 'Object':
                        ...
                        if (entity._additionalProperties && entity._additionalProperties.length > 0) {
                            schema.additionalProperties = obj2schema(entity._additionalProperties[0], models);
                        }
        ...
        function schema2obj(schema, key, required, isRoot, modelObjs, parent) {
            ...
            for (var x = 0; x < schema.type.length; x++) {
                    ...
                    case 'object':
                        ...
                        if (schema.hasOwnProperty('additionalProperties')) {
                            if (typeof schema.additionalProperties === 'boolean') {
                                ...
                                var additionalProp = schema2obj(schema.additionalProperties, 'additionalProperties', false, false, modelObjs);
                ...
                if (!type && schema.$ref) {
                    ...
                    var modelKey = '';
                    if (modelObjs) {
                        angular.forEach(modelObjs, function (model, _key) {
