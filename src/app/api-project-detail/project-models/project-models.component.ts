...
export class ProjectModelsComponent implements OnInit {
    ...
    selectModel(modelId: string) {
        ...
        if (this.modelForm.dirty) {
            this.confirmService.confirm({
                ...
                confirm: 'The Models view has some unsaved data. Do you want to replace it with your current selection?',
                confirmOk: 'Replace',
    ...
    private handleModelSelect(modelId: string) {
        ...
        if (modelId === 'NEW') {
            ...
            const { name, nameSpace, folder, data } = this.selectedPROJ.models[modelId];
            this.modelForm = this.fb.group({
    ...
    deleteModel(modelId: string) {
        if (!modelId || !this.selectedPROJ.folders)
            return;
    ...
    setDefaultNameSpace() {
        if (!this.modelForm.value.nameSpace)
            this.modelForm.patchValue({ nameSpace: this.modelForm.value.name.replace(/\s/g, '_') });
    }
    ...
    checkExistingModel(name) {
        ...
        var foundId;
        if (this.selectedPROJ.models) {
            this.selectedPROJ.models && Object.keys(this.selectedPROJ.models).forEach(id => {
                if (this.selectedPROJ.models[id].name.toLowerCase() === name.toLowerCase())
                    foundId = id;
            })
        }
        return foundId;
