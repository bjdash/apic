...
import { JsonSchemaSetArrayTypeEvent } from './field/selectschema.component';
import { SchemaClickOpenEvent } from './main/main.component';
...
export class JsonSchemaComponent implements OnInit, ControlValueAccessor, OnChanges {
  ...
  @Output() onTestBuilder = new EventEmitter<SchemaClickOpenEvent>()
  ...
  expand$ref(entity: Entity) {
    ...
    if (!obj._refExp) {
      ...
      var refData: Entity = this.JsonSchema.schema2obj(schema, '', undefined, true, this.models, obj._parent + (obj._key ? ('.' + obj._key) : '')); //obj._key is blank fro Array[$ref]
  ...
  setArrayType(event: JsonSchemaSetArrayTypeEvent) {
    var newM = this.generateModel(event.type, 'arrayEle');
    event.entity = this.selectedEntity;
    event.entity._items[0] = newM;
    if (event.type === '$ref') {
      this.configs.extraArrayOptn = true;
      this.modelRef = '';
    } else {
  ...
  displaySelectorModal(event?: SchemaClickOpenEvent) {
    if (event?.entity) {
      this.modelChangesCallback(event.entity, true);
    }

    if (event) {
      var winHeight = window.innerHeight;
      var left = event.clientX, bottom = winHeight - event.clientY;
