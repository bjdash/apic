...
export class SuitesStateSelector {
    ...
    static getProjectsPartial(projects: TestProject[]): TestProject[] {
        return projects.map(f => (({ _id, name, _created, _modified, owner }) => ({ _id, name, _created, _modified, owner }))(f));
    }
    ...
    static getSuitesPartial(suites: Suite[]): Suite[] {
        return suites.map(f => (({ _id, name, _created, _modified, projId, reqs, owner }) => ({ _id, name, _created, _modified, projId, reqs, owner }))(f));
    }
