...
import { LeftTreeFolder, ReqFolder } from "../models/ReqFolder.model";
import { ApiRequest, LeftTreeRequest } from "../models/Request.model";
...
export class RequestsStateSelector {
  ...
  static getReqsPartial(reqs: ApiRequest[]): LeftTreeRequest[] {
  ...
  static getFoldersTree(folders: ReqFolder[], reqs: LeftTreeRequest[]) {
  ...
  private static generateTree(folders: ReqFolder[], reqs: LeftTreeRequest[] | ApiRequest[]): LeftTreeFolder[] {
    // console.log('generating left tree', folders, reqs);
    var reqMap: { [key: string]: LeftTreeRequest[] } = {}; // map of requests based on the parent id
    reqs.forEach(r => {
      if (r._parent) {
        if (reqMap[r._parent]) {
    ...
    var folderMap: { [key: string]: LeftTreeFolder } = {};
    let folderTree = folders.filter(f => !f.parentId).map(f => {
      let rootFolder: LeftTreeFolder = { ...f, children: [], requests: reqMap[f._id] || [], treeItem: 'Folder' };
