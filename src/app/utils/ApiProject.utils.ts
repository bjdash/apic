...
import { TXT_TRAIT_IMPORT } from "./constants";
...
export class ApiProjectUtils {
  static importTraitData(traitId, endp: ApiEndp, project: ApiProject): ApiEndp {
    ...
    //import x-properties from trait
    let existingPropKeys = endp.xProperties.map(kv => kv.key);
    endp.xProperties = [
      ...((trait.xProperties || []).filter(kv => !existingPropKeys.includes(kv.key)).map(kv => {
        return { ...kv, readOnly: true, meta: TXT_TRAIT_IMPORT }
      })),
      ...endp.xProperties
    ]
    return endp;
  ...
  static removeTraitData(traitId: string, endpoint: ApiEndp, project: ApiProject): ApiEndp {
    ...
    //remove x-properties from trait
    let xProperties = endpoint.xProperties?.filter(kv => !kv.readOnly && kv.meta != TXT_TRAIT_IMPORT);
    return { ...endpoint, responses, pathParams, headers, queryParams, xProperties };
  }
