...
function getDemoData() {
    var data: any = {}, ts = 12345678;

    data.demoDesignProj = { "title": "APIC Todo demo", "version": "v2", _created: ts, _modified: ts, "description": "This is a set of sample `APIs` to demonstrate how [APIC's](https://apic.app) the [Designer](https://docs.apic.app/designer), [Tester](https://docs.apic.app/tester) (running a request, Test Suites etc) & [Docs](https://docs.apic.app/docs) work.", "contact": { "name": "APIC", "url": "https://apic.app/identity/#!/feedback", "email": "hello@apic.app" }, _id: ts + '-apiproj-demo', "tags": [{ "name": "todo", "description": "Tag for todo", "xProperty": [{ "key": "", "val": "" }], "externalDocs": { "url": "https://apic.app", "description": "Know more" } }], "securityDefinitions": [{ "type": "basic", "name": "basic_auth", "description": "Basic auth", "xProperty": [{ "key": "", "val": "" }] }, { "type": "apiKey", "name": "api_key", "description": "Api key authentication", "xProperty": [{ "key": "x-prefix", "val": "Bearer " }], "apiKey": { "name": "authorization", "in": "header" } }], "folders": { "1489390890584": { "_id": "1489390890584", "name": "Models", "desc": "This folder will hold all the models" } }, "models": { "1489394735056": { "_id": "1489394735056", "name": "Todo", "folder": "1489390890584", "nameSpace": "Todo", "data": { "type": "object", "properties": { "id": { "type": "string", "description": "Todo id" }, "name": { "type": "string", "description": "Todo name" }, "completed": { "type": "boolean", "description": "Completion status of the todo", "default": false }, "created": { "type": "integer", "description": "Todo creation timestamp" } }, "required": ["id", "name"] } } }, "setting": { "host": "apic.app", "basePath": "/apic-demo", "protocol": "https", envId: ts + '-env-demo' }, "endpoints": { "1489395152484": { "_id": "1489395152484", "operationId": "todo_create", "method": "post", "schemes": [{ "key": "https", "val": "HTTPS" }], "path": "/v2/todo", "folder": "", "tags": ["todo"], "security": [{ "name": "api_key" }], "traits": [], "summary": "Create Todo", "description": "The API to create a todo. Providing todo name is mandatory.", "pathParams": { "type": "object" }, "queryParams": { "type": "object" }, "headers": { "type": "object" }, "responses": [{ "code": "201", "desc": "To do successfully created. Newly created Todo details are in the response", "data": [{ "mime": "application/json", "schema": { "type": "object", "properties": { "todo": { "$ref": "#/definitions/Todo" }, "message": { "type": "string", "enum": ["Todo created successfully"] } }, "required": ["todo", "message"] }, "examples": [] }], "headers": { "type": "object", "properties": {}, "required": [] } }, { "code": "400", "desc": "Todo creation failed because Todo name was not provided", "data": [{ "mime": "application/json", "schema": { "type": "object", "properties": { "message": { "type": "string", "enum": ["Please provide a ToDo Name"] } }, "required": ["message"] }, "examples": [] }], "headers": { "type": "object", "properties": {}, "required": [] } }], "body": { "data": [{ "schema": { "type": "object", "properties": { "name": { "type": "string" } }, "required": ["name"] }, "mime": "application/json", "examples": [] }], "desc": "" }, "postrun": "apic.test(\"Check that Status code is 201 (Created)\", function(){\n\texpect($response.status).to.be.eql(201);\n});\napic.test(\"Response data should match the schema for status 201\", function(){\n\texpect($response).to.matchSchema(201);\n});\napic.test(\"$response.data.todo.name should be equal to $request.body.name\", function(){\n\texpect($response.data.todo.name).to.be.eql($request.body.name);\n});\napic.test(\"$response.data.message should be equal to \\\"Todo created successfully\\\"\", function(){\n\texpect($response.data.message).to.be.eql(\"Todo created successfully\");\n});\napic.test(\"$response.data.todo.id should exist in response\", function(){\n\texpect($response.data.todo).to.have.property(\"id\");\n});\napic.test(\"$response.data.todo.created should be a(n) date\", function(){\nexpect($response.data.todo.created).to.be.a.date;\n});\n\n\n//save the newly created Todo id in environment\nvar newId = $response.data.todo.id;\nlog(\"new todo id is: \"+newId);\nsetEnv(\"todoId\", newId);\n" }, "1489395400717": { "_id": "1489395400717", "operationId": "todo_get", "method": "get", "schemes": [{ "key": "https", "val": "HTTPS" }], "path": "/v2/todo/{todoId}", "folder": "", "tags": ["todo"], "security": [{ "name": "api_key" }], "traits": [], "summary": "Get Todo detail", "description": "The API to get the to do by providing its id in the path API path as a path parameter", "pathParams": { "type": "object", "properties": { "todoId": { "type": "string" } }, "required": ["todoId"] }, "queryParams": { "type": "object" }, "headers": { "type": "object" }, "responses": [{ "code": "200", "desc": "Todo retrieved successfully", "data": [{ "mime": "application/json", "schema": { "type": "object", "properties": { "todo": { "$ref": "#/definitions/Todo" }, "message": { "type": "string", "enum": ["Todo retrived successfully"] } }, "required": ["todo", "message"] }, "examples": [] }], "headers": { "type": "object", "properties": {}, "required": [] } }, { "code": "404", "desc": "No Todo found based on the given id", "data": [{ "mime": "application/json", "schema": { "type": "object", "properties": { "message": { "type": "string", "enum": ["Failed to fetch todo detail"] } } }, "examples": [] }], "headers": { "type": "object", "properties": {}, "required": [] } }], "postrun": "apic.test(\"Check that Status code is 200\", function(){\n\texpect($response.status).to.be.eql(200);\n});\napic.test(\"Response data should match the schema specified against status 200\", function(){\n\texpect($response).to.matchSchema(200);\n});\napic.test(\"$response.data.todo.id should be equal to environment variable todoId\", function(){\n\texpect( $response.data.todo.id).to.be.eql(getEnv(\"todoId\"))\n})" }, "1489395547584": { "_id": "1489395547584", "operationId": "todo_delete", "method": "delete", "schemes": [{ "key": "https", "val": "HTTPS" }], "path": "/v2/todo/{todoId}", "folder": "", "tags": ["todo"], "security": [{ "name": "api_key" }], "traits": [], "summary": "Delete Todo", "description": "", "pathParams": { "type": "object", "properties": { "todoId": { "type": "string" } }, "required": ["todoId"] }, "queryParams": { "type": "object" }, "headers": { "type": "object" }, "responses": [{ "code": "200", "data": [{ "mime": "application/json", "schema": { "type": "object", "properties": { "message": { "type": "string", "enum": ["Todo deleted"] } }, "required": ["message"] }, "examples": [] }], "headers": { "type": "object", "properties": {}, "required": [] } }, { "code": "404", "desc": "Couldn't find the specified Todo", "data": [{ "mime": "application/json", "schema": { "type": "object", "properties": { "message": { "type": "string", "enum": ["Couldn't find the specified todo"] } }, "required": ["message"] }, "examples": [] }], "headers": { "type": "object", "properties": {}, "required": [] } }], "body": { "data": [], "desc": "" }, "postrun": "apic.test(\"Check that Status code is 200\", function(){\n\texpect($response.status).to.be.eql(200)\n});\napic.test(\"Response data should match the schema specified against status 200\", function(){\n\texpect($response).to.matchSchema(200);\n});\napic.test(\"$response.data.message should be equal to \\\"Todo deleted\\\"\", function(){\n\texpect($response.data.message).to.be.eql(\"Todo deleted\");\n})" } } };
